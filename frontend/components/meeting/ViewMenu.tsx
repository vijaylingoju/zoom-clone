...
export function ViewMenu({ mode, hideSelf, onMode, onToggleHideSelf }: ViewMenuProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 rounded-lg bg-black/50 px-3 py-1.5 text-sm text-white hover:bg-black/70"
      >
        <LayoutGrid size={15} />
        View
      </button>
      {open && (
        <div className="absolute right-0 top-10 z-30 w-56 rounded-xl border border-white/10 bg-[#1f1f1f] p-1.5 shadow-2xl">
